    ...
    const session = yield* Effect.tryPromise({
      try: async () => {
        ...
        const { session } = await createAgentSession({
          ...
          // The host runtime accepts its shared registry even though the public
          // CreateAgentSessionOptions declaration does not expose it yet.
          // @ts-expect-error host-only session option
